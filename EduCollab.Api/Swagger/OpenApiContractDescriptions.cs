    ...
    public static class OpenApiContractDescriptions
    {
        ...
        public static string BuildInfoDescription()
        {
            return string.Join('\n',
                ...
                "- **Groups** `GET /api/workspace/groups`: optional `parentGroupId` to browse the group tree. Root browse (`parentGroupId` omitted) returns entry groups the caller may open (direct membership plus subgroups whose parent is not accessible). Drill down with `parentGroupId={groupId}`. Adding a user to a group also adds them to all subgroups.",
                ...
                "- **Scenes** `GET /api/workspace/scenes`: optional `owner=me`. Each scene has a single `groupId` placement. Create/update send inline `jsonContent` as JSON (`application/json`) or upload a `.json` file via `multipart/form-data` (`jsonFile` part). Scene objects reference workspace assets with an `assetId` property anywhere in the JSON tree.",
